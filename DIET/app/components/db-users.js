...
export async function updateUserProfile(userId, firstName, lastName, email, password, currentPassword = '', proteinGoal = 50, carbsGoal = 275, fatGoal = 78, allergens = '') {
	const url = `${getApiBase()}/updateUserProfile`;
	const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId, firstName, lastName, email, password, currentPassword, proteinGoal, carbsGoal, fatGoal, allergens }) });
	const json = await res.json();
